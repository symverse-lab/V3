...
## 5.1 Why the Legacy Model Is Included

Version `0` represents the **Legacy SymID** model.

The V3 specification uses the PQCFork-era Version `1` composition rule described in Section 4, but the Legacy SymID model remains important for:

- interpreting existing chain data,
- understanding reserved and system addresses,
- documenting historical account records,
- supporting migration and compatibility discussions.

This section preserves the important Legacy SymID structure without mixing it into the Version `1` SymID composition rule.
...
This Legacy account document belongs to the Version `0` SymID interpretation.
...
This issuer-oriented CA ID range belongs to the Version `0` Legacy model and is not part of the Version `1` PQCFork-era SymID composition rule.
...
These examples belong to the Version `0` Legacy model and should not be confused with the Version `1` PQCFork-era SymID composition rule.
